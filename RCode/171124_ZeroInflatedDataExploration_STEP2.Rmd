...
all.scenarios <- fread("/mnt/ifs1_projekte/genstat/02_projekte/1703_ge_metab_a1_b3_sorbs/171124_ZeroInflatedDataSim/results/171201_SimulationResults.csv")
...
all.scenarios[, .N, by = realization]
test.6 <- melt(data = all.scenarios,
               id.vars = c("effekt", "proz0", "categart", "categnum", "realization"),
               measure.vars = c("logit.pval"))

q.vs.b <- dcast.data.table(data = test.6, formula = effekt + proz0 + categnum + realization ~ categart, value.var = "value")
...
```{r compare.rank}
# boxplot of pvals for all linmod pvals <= 0.05 
test.5 <- melt(data = all.scenarios[linmod.pval <= 0.05, ],
               id.vars = c("effekt", "proz0", "categart", "categnum"),
