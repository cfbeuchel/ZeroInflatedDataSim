...
filename = "STEP 2: Zero-inflated data with zeros set to Min (minimum detection range) exploration"
```
...
newest.results <- newest_file(look_for = "SimulationResultsWithZeroSetToMinForSTEP2", subfolder = "results", print_full = T)
...
## Comparison of effect sizes between linear models
...
p_8(x = "linmod.beta.zero.inflated", y = "linmod.beta.full.data") +
  ...
  ggtitle(label = "Beta comparison",
          subtitle = "zero-inflated data ~ full data")
...
p_9(x = "linmod.pval.zero.inflated", y = "linmod.pval.zero.inflated.transformed.rescaled") + facet_wrap(~effekt , scales = "free") +
  ggtitle(label = "P-value comparison",
          subtitle = "zero-inflated untransformed data ~ zero-inflated IN-transformed & rescaled data")

# INT zero-inflated data vs spearman cor. of untransformed data
p_9(x = "spearman.cor.pval.zero.inflated", y = "linmod.pval.zero.inflated.transformed") + facet_wrap(~effekt , scales = "free") +
  ggtitle(label = "P-value comparison",
...
fit_plots(combat.data = combat.data, pdf_name = "zero_inflated_fit_NO_ComBat_with_Zeros_to_Detection_limit")

# with combat and transformation
fit_plots(combat.data = combat.data, pdf_name = "zero_inflated_fit_AFTER_ComBat_with_Zeros_to_Detection_limit")
