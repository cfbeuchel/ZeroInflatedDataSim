...
filename = "STEP 1: Zero-inflated data simulation with zeros set to min value"
```
...
all.combat.runs.raw <- mclapply(sort(unique(szenarien$combat.run)),  function(x) { # OUTER LOOP
  ...
  combat.data.raw <- lapply(my.combat.run$sub.run, function(i){ # INNER LOOP
    ...
    # way one:
    df[, y.0infl := y]
    df[y < quantile(y.0infl, probs = my.proz0/100), y.0infl := NA]
    min.found = df[y.0infl > 0, min(y.0infl)]
    df[is.na(y.0infl), y.0infl := min.found]
  ...
  cols.transformed <- names(transformed.cols)
  ...
  res <- purrr::map2(.x = combat.data[ , .SD, .SDcols = cols.to.transform],
              .y = combat.data[ , .SD, .SDcols = cols.transformed],
              .f = function(x, y) {
  ...
  # rename res cols to indicate rescailing
  names(res) <- paste0(names(res), ".transformed.rescaled")
  
  # write rescaled data into combat.data
  combat.data[ , names(res) := res]
  ...
  lm.cor.res <- lapply(my.combat.run$sub.run, function(f){
    ...
    cor.zero.inflated.transformed.rescaled <- tidy(combat.data[, cor.test(x = get(paste0("x.", f)), y = get(paste0("y.0infl.", f, ".transformed.rescaled")), method = "spearman")])
    ...
    res <- data.table(spearman.cor.full.data = cor.full.data$estimate,
                      ...
                      linmod.r2.zero.inflated.transformed.rescaled = linmod.zero.inflated.transformed.rescaled$r.squared,
                      ...
                      spearman.cor.zero.inflated.transformed.rescaled = cor.zero.inflated.transformed.rescaled$estimate,
                      spearman.cor.pval.zero.inflated.transformed.rescaled = cor.zero.inflated.transformed.rescaled$p.value)
...
save_csv_carl(file = all.scenarios,
              file_name = "SimulationResultsWithZeroSetToMinForSTEP2",
              subfolder = "results")
```
