...
toolboxH::parallelisiere()

# source small function
source(paste0(bp, "/genstat/02_projekte/1703_ge_metab_a1_b3_sorbs/functions/batch_anova.R"))
```
...
#=================================================================================#
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
all.combat.runs.raw <- lapply(sort(unique(szenarien$combat.run)),  function(x) { # OUTER LOOP
  ...
  combat.data.raw <- lapply(my.combat.run$sub.run, function(i){ # INNER LOOP
    ...
    dm <- MASS::mvrnorm(n = my.n,
                        mu = c(3,3), # the mean of the dristibution
  ...
  cols.to.transform <- names(combat.data)[grep("y.0infl.[0-9]+$", names(combat.data))] #[my.combat.run$proz0 != 0]
  ...
  # set colnames
  zero.infl.cols.transformed.rescaled <- names(res)
  ...
  # get cols 
  zero.infl.cols <- names(combat.data)[grep("y.0infl.[0-9]+$", names(combat.data))]
  ...
  zero.infl.pre.combat.after.transform.aov <- batch_anova(data = combat.data,
                                                          to.adjust = cols.transformed,
                                                          batch = "batch") 
  #-----------------------------------------------------------------
  #=================================================================
  ...
  combat.edata.zero.infl <- sva::ComBat(dat = my.edata.y.zero.infl,
                                        batch = my.batch,
                                        mod = my.modcombat,
                                        par.prior = F,
                                        prior.plots = T)
  ...
  ###
  # NEW AFTER CHANGING POSITION OF TRANSFORMATION - FROM CHANGING TRANSFORMATION ORDER
  ###

  # get the colums of the transformed data
  my.edata.y.zero.infl.transformed <- combat.data[, t(as.matrix(.SD)), .SDcols = cols.transformed]

  # combat on zero-inflated & transformed data data
  combat.edata.zero.infl.transformed <- sva::ComBat(dat = my.edata.y.zero.infl.transformed,
                                                 batch = my.batch,
                                                 mod = my.modcombat,
                                                 par.prior = F,
                                                 prior.plots = T)
  ...
  # reset pars
  par(mfrow=c(1,1))
  combat.data[, (cols.transformed) := as.data.table(t(combat.edata.zero.infl.transformed))]
  ...
  lm.cor.res[ , `:=`(num = my.combat.run$num,
                     ...
                     p.ANOVA.zero.infl.pre.ComBat.post.transformation = zero.infl.pre.combat.after.transform.aov$p.values, # NEW - IS IT NEEDED? #TAG#
