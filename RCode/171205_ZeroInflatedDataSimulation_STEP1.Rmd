...
#=================================================================================#
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
res <- mclapply(1:length(unique(szenarien$combat.run)[1:4]),  function(x) { # OUTER LOOP
  ...
  # get cols 
  zero.infl.cols <- names(combat.data)[grep("y.0infl", names(combat.data))]
  y.cols <- names(combat.data)[grep("^y.", names(combat.data))]
  ...
  # loop -------------------------------------------------
  lm.cor.res <- lapply(my.combat.run$sub.run, function(x){
    
    # formulas
    my.formula.full <- as.formula(paste0("y.", x, " ~ ", "x.", x))
    my.formula.0infl <- as.formula(paste0("y.0infl.", x, " ~ ", "x.", x))
    my.formula.0infl.transformed <- as.formula(paste0("y.0infl.", x, ".transformed ", " ~ ", "x.", x))
    ...
    cor.full.data <- tidy(combat.data[, cor.test(x = get(paste0("x.",x)), y = get(paste0("y.",x)), method = "spearman")])
    cor.zero.inflated <- tidy(combat.data[, cor.test(x = get(paste0("x.",x)), y = get(paste0("y.0infl.",x)), method = "spearman")])
    cor.zero.inflated.transformed <- tidy(combat.data[, cor.test(x = get(paste0("x.",x)), y = get(paste0("y.0infl.",x, ".transformed")), method = "spearman")])
  ...
  if(my.categart == "quantile") {
    ...
    categorised.cols <- combat.data[ , lapply(.SD, function(i){
      
      # i <- combat.data$y.0infl.45 # debug
      dummy <- data.table(data.no.categ = i)
  ...
  ### BAUSTELLE ######################
  #=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=
  lm.categ.res.raw <- lapply(my.combat.run$sub.run, function(x){
    
    # formulas for model definition
    dicho.val.name <- paste0("y.0infl.", x)
    categ.val.name <- paste0("y.0infl.", x, ".categorical")
    my.formula.linmod.dicho <- as.formula(paste0(dicho.val.name, "> 0 ~ ", "x.", x))
    my.formula.linmod.logit <- as.formula(paste0(categ.val.name," ~ ", "x.", x))
